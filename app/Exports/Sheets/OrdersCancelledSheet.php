...
class OrdersCancelledSheet implements FromCollection, WithHeadings, WithMapping, WithTitle, WithEvents, WithCustomStartCell
{
    ...
    {
        $query = $this->baseOrderQuery()->cancelledOnly();

        if ($this->status && $this->status !== 'отменен') {
            return collect();
        }
